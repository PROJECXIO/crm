...
const switchClasses = computed(() => {
	return [
		...
		props.modelValue
			? 'bg-[#068867] enabled:hover:bg-[#E4E6EC] active:bg-surface-gray-2'
			: 'bg-[#E4E6EC] enabled:hover:bg-[#55DDBB] active:bg-[#ec5757]',
...
const switchLabelClasses = computed(() => {
	return [
		'font-medium leading-normal text-[#666] ',
		props.disabled && switchType.value === SwitchVariant.ONLY_LABEL
			? 'text-[#666]'
			: 'text-[#666]',
		props.size === 'md' ? 'text-base' : 'text-sm',
	]
})
...
const switchGroupClasses = computed(() => {
	...
	if (switchType.value === SwitchVariant.ONLY_LABEL) {
		...
		classes.push(props.disabled ? 'cursor-not-allowed' : ' ')
