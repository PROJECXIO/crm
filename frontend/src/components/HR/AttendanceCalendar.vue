<template>
	<div class="flex flex-col gap-5 w-full md:w-112 mx-auto" v-if="calendarEvents.data">
		<div class="flex flex-col gap-6 bg-white py-6 px-3.5 rounded-lg border-none">
			...
			<div class="flex flex-row items-center justify-center gap-4 px-4">
				<Button
					icon="chevron-left"
					variant="ghost"
					class="!text-bg-green hover:!bg-transparent focus:!bg-none"
					@click="firstOfMonth = firstOfMonth.subtract(1, 'M')"
				/>
				<span class="text-lg text-bg-green">
			...
			<div class="grid grid-cols-7 gap-y-3">
				<div
					v-for="day in DAYS"
					class="flex justify-center text-subheading text-lg font-semibold leading-6"
				>
					{{ day }}
				</div>
				...
				<div v-for="index in firstOfMonth.endOf('M').get('D')">
					...
					>
						<span class="text-green-800 text-lg m-auto">
							{{ index }}
						</span>
					</div>
				</div>
			...
			<div class="grid grid-cols-4 gap-2 mx-2">
				<div v-for="status in summaryStatuses" class="flex flex-col gap-1">
					<div class="flex flex-row items-center px-1 rounded-sm" :class="colorMap[status]">
						<div class="flex items-center gap-1">
						<span class="text-gray-600 text-sm leading-5"> {{ __(status) }}: </span>
						</div>
						<span class="text-gray-800 text-base leading-6 mx-1">
						{{ summary[status] || 0 }}
					</span>
					</div>
