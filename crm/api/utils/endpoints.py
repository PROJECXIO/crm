...
from crm.api.utils import (
	delete_duplicated_or_after_error,
	format_data,
	get_request_form_data,
	upload_file,
)
from crm.api.utils.response import (
			...
			d.update(
				{
					"features": frappe.get_all(
						"Pet Service Feature Table",
						filters={"parent": name, "parentfield": "features", "parenttype": "Pet Service"},
						pluck="feature",
					)
				}
			)
...
def order_data_by_most_view(doctype, data, limit_page_length):
	...
	order_map = {order["reference_name"]: index for index, order in enumerate(most_viewed)}
...
def create_doc(doctype: str, default_data={}, ignore_permissions=False, ignore_links=False):
	uploaded_files = []
	doc = None
	try:
		...
		doc.insert(ignore_permissions=ignore_permissions, ignore_links=ignore_links)
		delete_duplicated_or_after_error(uploaded_files, ignore_permissions=ignore_permissions)
...
def handle_files(doc, ignore_permissions=False):
	...
	for field in meta.fields:
		if field.fieldtype not in ["Attach", "Attach Image"]:
			continue
		file_doc_name = upload_file(field.fieldname, ignore_permissions=ignore_permissions)
...
def update_doc(doctype: str, name: str, default_data={}, ignore_perms=False, keys_to_update=[]):
	...
	try:
		...
		doc.update(default_data)
		doc.save()
		delete_duplicated_or_after_error(uploaded_files, ignore_permissions=ignore_perms)
...
def delete_doc(doctype: str, name: str):
	try:
		...
		return handle_exception_response(None, doctype, exc, uploaded_files=[], for_delete=True)
