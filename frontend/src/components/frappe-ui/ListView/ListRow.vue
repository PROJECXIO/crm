<template>
	<component
		...
		class="flex flex-col transition-all duration-300 ease-in-out h-12"
		v-bind="{
	...
	>
		<component
			...
			class="[all:unset] hover:[all:unset] !h-full"
		>
			<div
				class="grid items-center space-x-4 rounded px-2 !h-full"
				:class="[isSelected ? 'bg-bg-green' : '', isHoverable && 'hover:bg-green-100']"
				:style="{
					height: rowHeight,
			...
			<div v-if="!isLastRow" class="mx-2 h-px border-t border-outline-gray-1" />
		</component>
	</component>
</template>
