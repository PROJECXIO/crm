        ...
        <template v-for="group in groups" :key="group.key">
          <div v-if="group.items.length" :class="cssClasses.groupContainer">
            ...
            >
              ...
              <button
                v-else
                :class="[cssClasses.itemButton, getBackgroundColor(item), '!p-2 hover:!bg-bg-green group !h-10 min-h-10']"
				style="padding: 20px"
              >
                <FeatherIcon
                  v-if="item.icon && typeof item.icon === 'string'"
                  :name="item.icon"
                  :class="[cssClasses.itemIcon, getIconColor(item), 'group-hover:stroke-white']"
                  aria-hidden="true"
                />
                <component
                  :class="[cssClasses.itemIcon, getIconColor(item), 'group-hover:!stroke-white ']"
                  ...
                  :is="item.icon"
                />
                <span :class="[cssClasses.itemLabel, getTextColor(item), 'group-hover:text-white p-2 !py-4']"
                  >{{ item.label }}
                </span>
              </button>
            </DropdownMenuItem>
...
import { ButtonProps } from 'frappe-ui'
